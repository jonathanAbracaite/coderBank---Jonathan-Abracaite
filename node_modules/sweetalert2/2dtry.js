...
class Usuario {
  constructor(DNI, nombre, contraseña, saldo, operaciones, PremioRecibido) {
    ...
    this.saldo = 0;
    this.operaciones = [];
    this.PremioRecibido = false || PremioRecibido;
  }
}
// ******* Objetos ************
...
function cargarSaldoAlStorage(user){
    localStorage.setItem("saldo", JSON.stringify(user.saldo));
}

function muestraDineroActual() {
    let montoActual = traerSaldodeLocalStorage();
...
function traerDatoPrimerIngresoLS(){
    const confirmacionPrimerLogin = localStorage.getItem("EsPrimerLogin")
  
    if(confirmacionPrimerLogin){
      confirmacion = JSON.parse(confirmacionPrimerLogin)
      return confirmacion
    }else{
      return false
    }
...
function transferir(user){
    btnAccionTransferir.addEventListener("click", (event) =>{
        let saldoActual = Number(traerSaldodeLocalStorage());
        let montoATransferir = parseInt(document.getElementById("monto").value)
        let destinoATransferir = document.getElementById("alias").value;
        event.preventDefault()
        traerUsuariosdeLocalStorage()
        if (montoATransferir === "" || montoATransferir < 0 ||isNaN(montoATransferir) && destinoATransferir === "") {
        ...
        Swal.fire({
            ...
            .then((result) => {
              ...
              if (result.isConfirmed) {
                ...
                muestraDineroActual(saldoActual);
                let saldoActualizado = muestraDineroActual(saldoActual)
                cargarSaldoAlStorage(user)
                traerSaldodeLocalStorage()
                cargarUsuariosALocalStorage()
                notificacionAlerta("success", "Excelente!", "La operación fue exitosa")
              }
          })
...
btnAccionSignup.addEventListener("click", (event) => {
  ...
  for (let i = 0; i < usuarios.length; i++) {
    if (usuarios[i].DNI === DNI) {
      ...
      notificacionAlerta("error", "No fue posible realizar el registro", "Ya existe una cuenta con ese DNI")
      // Detener el registro
      return;
    }
  }
  ...
  if (
    ...
    cargarUsuariosALocalStorage();
    notificacionAlerta("success", "Registro exitoso", "Ya podés ingresar a tu cuenta")
  }
  DNI.value = ""
  nombre.value = ""
  contraseña.value = ""
...
btnAccionLogin.addEventListener("click", (event) => {
  ...
  console.log(traerDatoPrimerIngresoLS())
  usuarios.forEach((usuario) => {
    if (usuario.DNI === DNILogin && usuario.contraseña === passLogin) {
      ...
      cargarUsuariosALocalStorage()
      cargarSaldoAlStorage(usuario)
      traerSaldodeLocalStorage(usuario)
      cargarEsPrimerIngresoLS(usuario)
      transferir(usuario)
